...
# instrucciones
echo "Para ejecutar este script solo usa: bash $0"
...
while true; do
    ...
    if command -v iwgetid &>/dev/null; then
        ssid=$(iwgetid -r)
        [ -z "$ssid" ] && ssid="(N/A)"
        signal=$(iwconfig 2>/dev/null | grep -i 'Signal level' | awk -F'=' '{print $3}' | awk '{print $1}')
        [ -z "$signal" ] && signal="N/A"
    else
    ...
    if [ -d /sys/class/power_supply/BAT0 ]; then
        ...
        battery="$batt_capacity% ($batt_status)"
    else
        battery="N/A"
    fi
    ...
    printf "${c_iface}%-10s${c_reset} %s\n" "Interface:" "$iface_default"
    printf "${c_wifi}%-10s${c_reset} %s\n" "WiFi SSID:" "$ssid"
    printf "${c_signal}%-10s${c_reset} %s\n" "Signal:" "$signal"
    printf "${c_batt}%-10s${c_reset} %s\n\n" "Battery:" "$battery"
